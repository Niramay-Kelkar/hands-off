...
**The claim was then pushed further: not just locator targeting, but the full checkpoint/outcome/escalation lifecycle.** `target_app_tenant_b` gained one more fixture ID, `20004`, mirroring `target_app`'s `10004` interstitial pattern exactly — a fabricated (non-DB) search result whose detail page renders an unexpected `role="dialog"` modal. The same untouched artifact was replayed against it and escalated via the identical trigger tenant A's own escalation evidence uses (`on_unrecognized_dialog`), paused with a redacted evidence screenshot, was resumed by a second Playwright process attaching to the SAME live browser over CDP to dismiss the modal (standing in for a human, the identical method already proven during the original engine build) followed by a real `POST /resume/<run_id>` against a live `agent.operator_console` process, and completed cleanly: `status: "success"`, `member_name: "Marcus Webb"`. Every mechanism this exercises — dialog-interrupt detection, the escalation policy's retry/escalate rule, the SQLite session handoff, the operator console's resume endpoint — is the exact same code path used for tenant A's `escalation_10004/`, unmodified. Evidence saved at `evidence/replays/cross_tenant_b_escalation_20004/`; full narrative for both rounds in `BUILD_LOG.md`.

This was, deliberately, a single-session, three-case test (happy path, not-found, escalation) — not the full five-outcome matrix tenant A has (access-denied and slow-load were not re-seeded on tenant B; see `target_app_tenant_b/README.md`). The result converts the multi-tenant claim from "untested bet" to "tested across the taxonomy's three structurally distinct branches (success, business outcome, escalation), passed cleanly," not a claim that every fault path is tenant-agnostic. The underlying detection mechanism for when the bet stops holding is unchanged and still real: every locator resolution attempt, including which ranked strategy actually succeeded, is logged. A capability that begins consistently falling through to a lower-priority strategy, or fails to resolve at all, is a concrete operational signal that a given tenant's version has drifted — the trigger for re-recording or authoring a tenant-specific override, rather than something failing silently in production.
